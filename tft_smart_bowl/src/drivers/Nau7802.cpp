...
Core::Result<void> Nau7802::begin() {
  ...
  // NAU7802 begin() does a lot of I2C transactions (reset, power up, LDO
  // config, AFE calibration) We must lock the bus for the entire duration so
  // other threads don't interrupt it.
  if (!i2c.lock(2000)) { // Give it up to 2 seconds to acquire lock, as begin()
                         // can take ~300ms
  ...
  if (!success) {
    ...
    return Core::Result<void>(Core::Error(Core::ErrorCategory::I2C,
                                          Core::ErrorCode::DeviceNotFound,
                                          "NAU7802 not detected"));
  }

  // Optionally set specific gain/rate here, though default is Gain 128,
  // LDO 3.3V, 10SPS
...
Core::Result<int32_t> Nau7802::getReading() {
  if (!_initialized) {
    return Core::Result<int32_t>(Core::Error(Core::ErrorCategory::System,
                                             Core::ErrorCode::InvalidState,
                                             "NAU7802 not initialized"));
  }
