...
void WeightService::begin() {
    ...
    // The NAU7802 must have been initialized by Application::setup / BoardSelfTest
    // before this task starts, but we ensure the driver is ready.
                ...
                if (_calibState != CalibState::Idle) {
                    if (stable) {
                        ...
                        if (millis() > _calibTimeout) {
                            LOG_WARN("WGT", 604, "Calibration timeout — scale did not stabilize");
                            _calibState = CalibState::Idle;
                        }
                    }
                }
...
float WeightService::getWeight() const {
    return _currentWeightGrams;
}

bool WeightService::isStable() const {
    return _stability.isStable();
}

bool WeightService::isCalibrated() const {
...
void WeightService::requestSpan(float referenceGrams) {
    if (_calibState != CalibState::Idle) return;
    LOG_INFO("WGT", 606, "Span requested for %.1fg. Waiting for stability...", referenceGrams);
