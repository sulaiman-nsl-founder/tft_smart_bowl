...
Calibration::Calibration() { resetToDefaults(); }
...
void Calibration::setSpan(int32_t rawSpan, float refGrams) {
  ...
  _data.crc = calculateCrc(_data);
  _valid = true;
  LOG_INFO("CAL", 502, "Span set: %d raw -> %.1f g (factor: %f)", rawSpan,
           refGrams, _data.calibrationFactor);
}
...
bool Calibration::load() {
  ...
  _data = loadedData;
  _valid = true;
  LOG_INFO("CAL", 506, "Loaded calibration: zero=%d, factor=%f", _data.rawZero,
           _data.calibrationFactor);
  return true;
}
...
bool Calibration::isValid() const { return _valid; }
